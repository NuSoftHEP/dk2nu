...
void calcLocationWeights(dkmeta* dkmetaObj, dk2nu* dk2nuObj)
{
  ...
  for (size_t iloc = 0; iloc < nloc; ++iloc ) {
    ...
    TVector3 xyzDet(dkmetaObj->xloc[iloc],
                    dkmetaObj->yloc[iloc],
                    dkmetaObj->zloc[iloc]);  // position to evaluate
    double enu_xy = 0;  // give a default value
    double wgt_xy = 0;  // give a default value
    int status = CalcEnuWgt(dk2nuObj,xyzDet,enu_xy,wgt_xy);
    if ( status != 0 ) {
      std::cerr << "CalcEnuWgt returned " << status << " for " 
                << dkmetaObj->nameloc[iloc] << std::endl;
    }
    // with the recalculated energy compute the momentum components
    TVector3 xyzDk(dk2nuObj->vx,dk2nuObj->vy,dk2nuObj->vz);  // origin of decay
    TVector3 p3 = enu_xy * (xyzDet - xyzDk).Unit();
    dk2nuObj->nupx.push_back(p3.x());
    dk2nuObj->nupy.push_back(p3.y());
    dk2nuObj->nupz.push_back(p3.z());
    dk2nuObj->nuenergy.push_back(enu_xy);
    dk2nuObj->nuwgt.push_back(wgt_xy);
  }
}
               ...
               double& enu, double& wgt_xy)
{
  ...
  case kpdg_omegaplus:
    ...
    break;
  default:
    std::cerr << "CalcEnuWgt unknown particle type " << dk2nuObj->ptype
              << std::endl << std::flush;
    assert(0);
    return 1;
  }

  double parentp2 = ( dk2nuObj->pdpx*dk2nuObj->pdpx +
                      dk2nuObj->pdpy*dk2nuObj->pdpy +
                      dk2nuObj->pdpz*dk2nuObj->pdpz );
  ...
  double enuzr = dk2nuObj->necm;
  // Get angle from parent line of flight to chosen point in beam frame
  double rad = TMath::Sqrt( (xpos-dk2nuObj->vx)*(xpos-dk2nuObj->vx) +
                            (ypos-dk2nuObj->vy)*(ypos-dk2nuObj->vy) +
                            (zpos-dk2nuObj->vz)*(zpos-dk2nuObj->vz) );
  ...
  if ( parentp > 0. ) {
    costh_pardet = ( dk2nuObj->pdpx*(xpos-dk2nuObj->vx) +
                     dk2nuObj->pdpy*(ypos-dk2nuObj->vy) +
                     dk2nuObj->pdpz*(zpos-dk2nuObj->vz) ) 
                     / ( parentp * rad);
  ...
  double sangdet = ( kRDET*kRDET / 
                     ( (zpos-dk2nuObj->vz)*(zpos-dk2nuObj->vz) ) ) / 4.0;
  ...
  if ( dk2nuObj->ptype == kpdg_muplus || dk2nuObj->ptype == kpdg_muminus) {
    double beta[3], p_dcm_nu[4], p_nu[3], p_pcm_mp[3], partial;

    // Boost neu neutrino to mu decay CM
    beta[0] = dk2nuObj->pdpx / parent_energy;
    beta[1] = dk2nuObj->pdpy / parent_energy;
    beta[2] = dk2nuObj->pdpz / parent_energy;
    p_nu[0] = (xpos-dk2nuObj->vx)*enu/rad;
    p_nu[1] = (ypos-dk2nuObj->vy)*enu/rad;
    p_nu[2] = (zpos-dk2nuObj->vz)*enu/rad;
    partial = gamma * 
    ...
    double particle_energy = dk2nuObj->ppenergy;
    gamma = particle_energy/parent_mass;
    beta[0] = dk2nuObj->ppdxdz * dk2nuObj->pppz / particle_energy;
    beta[1] = dk2nuObj->ppdydz * dk2nuObj->pppz / particle_energy;
    beta[2] =                    dk2nuObj->pppz / particle_energy;
    partial = gamma * ( beta[0]*dk2nuObj->muparpx + 
                        beta[1]*dk2nuObj->muparpy + 
                        beta[2]*dk2nuObj->muparpz );
    partial = dk2nuObj->mupare - partial/(gamma+1.0);
    p_pcm_mp[0] = dk2nuObj->muparpx - beta[0]*gamma*partial;
    p_pcm_mp[1] = dk2nuObj->muparpy - beta[1]*gamma*partial;
    p_pcm_mp[2] = dk2nuObj->muparpz - beta[2]*gamma*partial;
